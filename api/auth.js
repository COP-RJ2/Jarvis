/**
 * Verificação em 2 etapas do JARVIS via SeaTalk (pedido do Roberto em
 * 2026-09-14) — 1º fator: e-mail do domínio corporativo. 2º fator: código
 * de 6 dígitos mandado por DM do bot do SeaTalk, sem senha nenhuma.
 *
 * Fluxo:
 ...
 *
 * Os códigos ficam no Postgres (tabela auth_codes, criada sozinha no
 * primeiro uso) — mesmo pool de db.js que o sync histórico já usa. Sessão
 * em si é responsabilidade do server.js (express-session); este arquivo só
 * seta/lê req.session.user.
 */
const { pool } = require('../db');
const { emailPermitido, usuarioDoEmail } = require('./_users');
const { resolverEmployeeCodePorEmail, enviarMensagemDireta } = require('./_seatalk');
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    res.status(502).json({ ok: false, erro: 'Não foi possível enviar o código pelo SeaTalk agora. Tenta de novo em instantes.' });
    return;
  }
...
module.exports = async (req, res) => {
  try {
    ...
    res.status(400).json({ ok: false, erro: 'Use ?request=1, ?verify=1, ?logout=1 ou ?me=1' });
  } catch (err) {
